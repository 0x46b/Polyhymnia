...
void init_style_dropdown_style_MAIN_DEFAULT(lv_style_t *style) {
    lv_style_set_text_color(style, lv_color_hex(0x989790));
};

lv_style_t *get_style_dropdown_style_MAIN_DEFAULT() {
    static lv_style_t *style;
    if (!style) {
        style = (lv_style_t *)lv_malloc(sizeof(lv_style_t));
        lv_style_init(style);
        init_style_dropdown_style_MAIN_DEFAULT(style);
    }
    return style;
};

void add_style_dropdown_style(lv_obj_t *obj) {
    (void)obj;
    lv_obj_add_style(obj, get_style_dropdown_style_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};

void remove_style_dropdown_style(lv_obj_t *obj) {
    (void)obj;
    lv_obj_remove_style(obj, get_style_dropdown_style_MAIN_DEFAULT(), LV_PART_MAIN | LV_STATE_DEFAULT);
};
