...
void encoder_initialize(volatile encoder_config *config, volatile uint8_t *pin,
                        volatile uint8_t *ddr, volatile uint8_t *port,
                        uint8_t phaseA, uint8_t phaseB) {
...
void oscillator_tick(volatile oscillator_controller *controller) {
  ...
  controller->state.DetuneOffset +=
      encoder_read_offset(&controller->encoderDetune);
  controller->state.Type =
      handle_type_change(encoder_read_offset(&controller->encoderType));
}
