...
LadderFilter* Filter;
Settings settings;
SerialLogger logger("Polyhymnia", LOGLEVEL_TRACE);
...
void initialize_audio_system(){
  logger.print("Initializing Audio System...", LOGLEVEL_DEBUG);
  AudioMemory(20);
  ...
  logger.println("[OK]", LOGLEVEL_DEBUG);
}

void initialize_synthesizer_system(Settings settings){
  logger.print("Initializing synthesizer system...", LOGLEVEL_DEBUG);
  PolyVCO1 = new VCO(&VCO1, 70, 76, 0.25, "VCO 1");
  PolyVCO2 = new VCO(&VCO2, 77, 78, 0.25, "VCO 2");
  PolyVCO3 = new VCO(&VCO3, 85, 86, 0.25, "VCO 3");
  Mixer = new SynthMixer(&VCOMixer, 20, 21, 22, 23, "Mixer");
  Envelope = new SynthEnvelope(&ADSR, 73, 80, 75, 72, "Envelope");
  Filter = new LadderFilter(&VCF);

  if(settings.is_loaded()){
    logger.println("Initialize Synthesizer: Using loaded settings", LOGLEVEL_DEBUG);
    ...
    logger.println("Settings applied", LOGLEVEL_DEBUG);
  }else {
    logger.println("Initialize Synthesizer: Using default-values", LOGLEVEL_DEBUG);
    PolyVCO1->Initialize();
    PolyVCO2->Initialize();
    PolyVCO3->Initialize();
    Mixer->Initialize();
...
bool initialize_sd_card(){
  logger.print("Initializing SD-card...", LOGLEVEL_DEBUG);
  
  // see if the card is present and can be initialized:
  if (!SD.begin(chipSelect)) {
    logger.println("[FAILED]: Card failed, or not present", LOGLEVEL_ERROR);
    return false;
  }
...
void intialize_settings(){
  logger.print("Loading settings...", LOGLEVEL_DEBUG);
...
void setup() {
  ...
  logger.print("Setting status-led to communicate ready...", LOGLEVEL_DEBUG);
  pinMode(ledPin, OUTPUT);
  digitalWrite(ledPin, HIGH);  // set the LED on
  logger.println("[OK]", LOGLEVEL_DEBUG);

  logger.println("Initializing [Done]", LOGLEVEL_DEBUG);
}
